...
class MemberRanksService {
  static async execute(collection_cd?: number, mbrid?: number, limit?: number, yearsAgo?: number) {
    const currentDate = new Date();
    const startDate = yearsAgo
      ? new Date(currentDate.getFullYear() - yearsAgo, currentDate.getMonth(), currentDate.getDate())
      : undefined;

    const allRecords = await prisma.biblioStatusHist.findMany({
      where: {
        ...(collection_cd ? { biblio: { collection_cd } } : {}),
        ...(startDate ? { status_begin_dt: { gte: startDate } } : {}),
      },
      select: {
        mbrid: true,
        bibid: true,
    ...
    const globalMemberStats = allRecords.reduce((acc, { mbrid, status_begin_dt }) => {
      ...
      if (!acc.has(mbrid)) {
        ...
        acc.set(mbrid, {
          ...
          earliestDate:
            status_begin_dt < current.earliestDate
              ? status_begin_dt
    ...
    const globalRanking = globalRankedResults.map((member, index, array) => {
      if (
        ...
        (member.totalBooksBorrowed !== array[index - 1].totalBooksBorrowed ||
          member.earliestDate.getTime() !==
            array[index - 1].earliestDate.getTime())
      ) {
        currentRank = index + 1;
      }
      return {
